...
const limits = {
    width: { min: 9, max: 25 }, // Новий діапазон для ширини
    height: { min: 9, max: 25 }, // Новий діапазон для висоти
};
...
function init() {
    ...
    createPillow(15, 15); // Створимо квадратну подушку з початковими розмірами
...
function createPillow(width, height) {
    if (pillowGroup) {
    ...
    // Створення геометрії для квадратної подушки
    const geometry = new THREE.BoxGeometry(width, height, 1); // Прямокутник (висота 1)
...
// Функція для обчислення динамічного коефіцієнта (не змінюється)
